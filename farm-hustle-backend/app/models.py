...
from sqlalchemy import String, Integer, Float, ForeignKey, DateTime, Boolean, JSON, UniqueConstraint, Text
...
Копировать
buildings: Mapped[list["UserBuilding"]] = relationship(back_populates="user", cascade="all, delete-orphan")
boosts: Mapped[list["UserBoost"]] = relationship(back_populates="user", cascade="all, delete-orphan")
class UserBuilding(Base):
tablename = "user_buildings"
id: Mapped[int] = mapped_column(primary_key=True)
user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True)
building_id: Mapped[str] = mapped_column(String(32), index=True) # 'grain','dairy', etc.
...
prize_pool_final_usd: Mapped[float] = mapped_column(Float, default=0.0) # after finalize
created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
