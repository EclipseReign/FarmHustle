...
SET_BONUSES = [
    {
        "key": "dairy",
        "label": "Cheesemaker’s Guild",
        "required": ["grain", "dairy"],
        "multiplier": 1.10,
    },
    {
        "key": "grove",
        "label": "Sunrise Orchard",
        "required": ["grove", "apiary"],
        "multiplier": 1.08,
    },
]
...
def ensure_user_buildings(db: Session, user: User):
    ...
    for bid in CATALOG.keys():
        if bid not in have:
            db.add(
                UserBuilding(
                    user_id=user.id, building_id=bid, level=1 if bid == "grain" else 0
                )
            )
    db.flush()
...
def current_income_per_sec(user: User) -> float:
    ...
    return (
        base
        * active_set_multiplier(user.buildings)
        * max(1.0, user.season_multiplier_paid)
    )
