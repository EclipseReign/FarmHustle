...
# top 10
top = db.query(Leaderboard, User).join(User, Leaderboard.user_id==User.id).order_by(Leaderboard.skill_score.desc()).limit(10).all()
dist = []
...
db.commit()
return {"season_id": s.id, "pool_final_usd": s.prize_pool_final_usd, "distribution": dist}
