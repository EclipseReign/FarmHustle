...
from ..economy import CATALOG, upgrade_cost, current_income_per_sec, ensure_user_buildings, sets_active, skill_score_proxy
...
@router.post("/rotate")
def rotate(authorization: str | None = Header(default=None), db: Session = Depends(get_db)):
...
ub = UserBoost(user_id=user.id, label="Hot Zone", multiplier=3.0, ends_at=ends, targets={"ids": targets})
user.boosts_used += 1
db.add(ub)
...
def upgrade(data: UpgradeIn, authorization: str | None = Header(default=None), db: Session = Depends(get_db)):
user = require_user(db, authorization)
accrue_offline(user)
ensure_user_buildings(db, user)
...
def prestige(authorization: str | None = Header(default=None), db: Session = Depends(get_db)):
user = require_user(db, authorization)
accrue_offline(user)
