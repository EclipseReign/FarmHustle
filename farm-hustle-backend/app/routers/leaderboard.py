...
def top(authorization: str | None = Header(default=None), db: Session = Depends(get_db), limit: int = 50):
uid = get_uid(authorization)
q = db.query(Leaderboard, User).join(User, Leaderboard.user_id==User.id).order_by(Leaderboard.skill_score.desc()).limit(limit)
top = [LeaderboardItem(name=u.name, skill_score=lb.skill_score) for lb, u in q.all()]
you = None
your_rank = None
if uid:
