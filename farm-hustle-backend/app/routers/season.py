...
now = datetime.utcnow()
s = db.query(Season).filter(Season.status=="active").order_by(Season.id.desc()).first()
...
pool_preview = max(min(pool_pct, settings.PRIZE_POOL_CAP_USD), settings.PRIZE_POOL_MIN_USD) if s.net_revenue_usd>0 else settings.PRIZE_POOL_MIN_USD
return {
"id": s.id,
